...
"*root@*" { send "if test ! -e .ssh;then mkdir .ssh;chmod 700 .ssh;fi;if test ! -e \".ssh/authorized_keys\";then touch \".ssh/authorized_keys\";chmod 600 \".ssh/authorized_keys\";fi;echo $pubkey >> \".ssh/authorized_keys\";\r"}
}
send "logout\r"
interact
